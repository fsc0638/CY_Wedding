...
## 2026-07-27 喜餅核銷：統計改「盒數」＋西式/雙份標記大幅強化
- 回報：人數正確但喜餅數量要調整（正確為**中式 41 盒、西式 4 盒**）；且西式／中式+西式的標記不夠清楚，
  核銷當下不易看出該位要領哪一種。
- 資料一致性驗證：`guests.json` 現為 **42 位**女方發券者。由
  `中式盒數=只中式+兩者=41`、`西式盒數=只西式+兩者=4`、`總人數=42` 解聯立
  → **只中式 38、只西式 1、中式+西式 3**（42 人 / 45 盒），與使用者數字完全自洽
  → 判定 Firestore 資料正確，問題在**顯示與統計口徑**（原本統計的是「人數分類」而非「盒數」）
  - ⚠️ 註：本機 data/ 的 xlsx 仍是 0630 舊檔（23 筆），最新名單在 Mac；本次不需重跑資料
- 改動（`verify.html`）：
  - **統計改盒數**（「中式+西式」者兩邊各算 1 盒）：
    名單統計 → `已核銷 X/42 人｜中式 a/41 盒 · 西式 b/4 盒｜其中 3 人領兩盒`；
    總覽 KPI → `中 a/41 · 西 b/4`；CSV 統計列同步
  - **核銷面板**：`both` 直接列出兩個大徽章「中式喜餅 ×1」「西式喜餅 ×1」；
    新增 `.gift-alert` 醒目警示條 —— both →「⚠ 這位需領『兩盒』」、western →「⚠ 這位只領『西式』1 盒（不是中式）」
  - **標記強化**（西式/雙份是少數、最容易發錯）：`.badge-west` 改橘金底 #e8a317 + 深字 + 粗體800 + 邊框；
    `.badge-both` 改紫底白字 + 粗體800 + 邊框；中式維持低調（多數）
  - **名單列**：非純中式者整列淡底（`.g-west` #fffaf0、`.g-both` #faf5fd），長列表中一眼可辨；
    雙份徽章文字改「中式+西式 2盒」
  - 面板大徽章字級 0.92→1.05rem
- ✅ 驗證：**直接抽取線上檔案內的統計程式碼**對測資（38/1/3）執行 → 中式 41 盒、西式 4 盒、
  已領中式 3、已領西式 1、領兩盒 3 人，全部正確；徽章/警示/列底色的 computed style 皆符合預期；無 console error
- ⏳ 待真機確認：登入後台看統計是否顯示「中式 __/41 盒 · 西式 __/4 盒」——若與 41/4 不符，代表 Firestore 內
  某幾位的「喜餅類型」需修正（改 Excel 後在 Mac 重跑 `deploy_roster.py`）
...
- 改為 `2026-09-12T12:00:00+08:00`；實測畫面顯示 46 天 22 時，與 12:00 目標相符

### ② 版權標示遮住新人額頭（手機版）
- 回報：版權文字壓在主角額頭上。**量測確認**（手機 375×812）：
  版權 494–530、頭頂 509 → 確實重疊；且「See you Soon!」底部到頭頂**僅 41px** 可用空間
- 頭部位置推算：背景照 `DSC08501.jpg`（2048×1365），新人頭頂約在圖高 **58.5%**；
  依 `background-size: cover` + `background-position: center 46%` 換算出各尺寸下頭部的螢幕座標
- 處置（`styles.css`）：
  - `.seeyou-copy`：`margin-top` 2.4rem → **0.7rem**、`line-height` 1.9 → **1.65**（壓縮成窄帶）
  - `.seeyou-inner`：加 `transform: translateY(-8vh)` → 整體上移，讓「See you Soon!」與版權都落在頭部上方空白處
  - 上移量取 8vh 是因為**桌機才是最嚴苛情境**：1280×800 時字體 96px 會換行成兩行（區塊高 209px），
    -4vh 仍重疊 18px；-8vh 後三種尺寸皆淨空
- ✅ 量測驗證（皆未遮頭、未與頁尾後台連結重疊、文字仍在 footer 內）：
  | 裝置 | 版權 | 頭頂 | 淨空 |
  |---|---|---|---|
  | 手機 375×812 | 424–456 | 509 | 53px |
  | 平板 768×1024 | 555–587 | 642 | 55px |
  | 桌機 1280×800 | 470–502 | 516 | 14px |
- 註：此環境截圖工具不可用（Browser pane 未顯示），全程以幾何量測驗證；實際觀感仍建議真機看一眼

## 2026-07-27 核銷驗證碼「依日期自動換碼 + 自動隱藏」（0638 → 1129）
- 需求：原設計「活動開始前三天變更驗證碼」希望能**自動**完成、不用當天手動改。
- 先釐清的關鍵矛盾：彈窗原本**直接顯示**【核銷驗證碼：0638】，下一行卻寫「請聯繫主辦單位索取」
  → 在碼公開顯示的狀態下換碼**毫無安全意義**。使用者決定：換碼同時**把碼藏起來**、新碼 `1129`。
- 實作（`script.js`，取代原 `UNLOCK_CODE` 常數）：
  ```js
  ...
  ```
  - `syncCodeHint()`：換碼前顯示碼；**換碼後 `codeHint.hidden = true`**（賓客改向主辦單位索取，與彈窗文案一致）
  - `submitCode()` 比對改用 `unlockCode()`（函式而非常數 → 頁面跨越切換時點仍正確）
  - 用**絕對時間點 +08:00**（非裝置本地日期）→ 全球同一瞬間切換
  - `styles.css` 加 `.vm-code[hidden] { display: none; }`（沿用檔內既有 `[hidden]` 寫法）
- 為何選「前端日期切換」而非 GitHub Actions 排程：效果相同但複雜度低得多；Firestore 方案（可隨時改碼、免部署）
  也評估過，但前端要讀取＝一樣可被讀到，對此軟性機制無實益。
- ⚠️ 誠實限制：新碼 `1129` 仍寫在 `script.js` 原始碼中，懂技術者可查看 → **這是軟性防呆，不是權限控管**。
  真正的重複領取防護是 Firestore `redeemed` 狀態 + 工作人員掃碼核銷（撕票僅為本機視覺狀態）。
  若要真正隱藏需後端驗證（Cloud Functions），評估為過度工程。
- ✅ 驗證（本機瀏覽器，**兩個分支都實測**）：
  - 換碼前（今日 2026-07-27）：顯示【核銷驗證碼：0638】；輸入 1129 被擋並顯示錯誤、輸入 0638 解鎖並復原券
  - 換碼後（暫時把切換點改為過去日期實測，測後已還原並 diff 確認無殘留）：碼提示 `display:none`；
    輸入舊碼 0638 被擋、輸入新碼 1129 解鎖並復原券
- 資產版本 bump → `?v=20260727b`
- 📌 待辦提醒：彈窗註記 `voucher.unlock.note`「此驗證碼將於活動開始前三天進行變更」在 09-09 後語意略顯過時
  （仍無害，未改動以降低風險）；若在意可屆時改為「請聯繫主辦單位索取」。
...
## 2026-09-10 新增 Video/：文定儀式 75 吋電視播放的「新聞快報」風影片
- 需求：文定當天電視循環播放，版面參考台灣新聞台（左下地區/天氣欄 + 大標題帶 + 跑馬燈），
  只借版面與風格、不照抄原圖文；右上角換成 CY 婚禮 logo
- 新增 `Video/make_news_video.py`：Pillow 逐格合成 → rawvideo 管線餵 ffmpeg，中間幀不落地
  - 版面座標一律用「佔畫布寬/高的比例」（同 make_vouchers.py 的做法），`--scale 2` 直接產 4K 不跑版
  - 直幅婚紗照兩張並排（對應參考版面的雙圖比較）、橫幅照滿版，每張帶緩慢 Ken Burns
  - 段落交叉溶接，且**最後一段溶回第一段** → 設重複播放無接縫
  - 太陽 icon 以程式畫（圓 + 八道光芒、4× 超取樣），免素材、任何解析度都銳利
  - 字體：PingFang 在近期 macOS 移到 `AssetsV2`（路徑帶雜湊）→ 用 glob 找，退路 Hiragino / STHeiti
  - `--still N`／`--at 秒數` 出單張 PNG，校版面與轉場用（不必等整片產完）
- 文字內容全在 `Video/config.json`：16 則快報標題、跑馬燈、左下欄（桃園區 / 新潟 / 14-21 / 太陽 icon）
- 配色沿用 styles.css 的婚禮色票，新聞台大紅改成深酒紅 `#4a1a21` + 金 `#f2c54e`，有快報感但不失禮
- 產片踩到兩個坑（已修）：
  - ①跑馬燈字被蓋淡：原本每格把整條跑馬燈框架（含 238 alpha 奶油底）重疊回最上層以蓋住標籤，
    連捲動的字一起蓋掉 → 改成只補回左端標籤那一小塊
  - ②轉場糊掉：標題帶原本連底色一起淡入，半透明時照片透出來一片混濁 →
    標題帶底色移進常駐框架，只讓兩行大標滑入（並裁掉溢出帶外的部分，免得壓到跑馬燈）
- 成品 1080p / 30fps / H.264 High@4.1 / 無聲 / 1:52 / 約 62 MB，產片約 80 秒
  - 無聲是刻意的：現場自有音樂，電視靜音播即可；需要配樂用 `--bgm` 併入
- `Video/out/` 已加入 .gitignore（單檔數十 MB、可隨時重產）
- 追加：依回饋移除畫面正中央的大字（原本模仿參考圖的「場景超雷同」）。婚紗照不再被壓住、畫面更乾淨；
  `config.json` 的 `cap` 改名 `note`，退化成純段落備註（不會上畫面），程式端一併移除 `build_caption` 與相關版面常數
- 追加（第二輪回饋）：
  - 畫面時鐘起始改 `07:40:00`（文定實際開始時刻）
  - 右上角字樣「文定 LIVE」→「LIVE」
  - 大標由兩行改**單行**：標題帶高度隨之收窄（`R_BAND_TOP` 0.731→0.805）、字級放大（`R_F_HEAD` 0.072→0.088）、
    滑入距離縮短（0.055H→0.028H，否則矮帶子會把整行裁掉）；`config.json` 的 `l1`/`l2` 合併為 `headline`
    ※ 不是單純砍掉第二行——多數段落的笑點在第二行，改成把兩行濃縮成一句（控制在 20 字內以維持字級）
  - 移除「男方提親『誠意滿滿』」那則 → 15 則、總長 1:45
- 追加（第三輪回饋）：改成文定實際時段 08:00–08:40 的 40 分鐘單檔，並加入主角取景
  - 素材換成 `Video/photos/`（使用者另外提供的 25 張婚紗照，原本只有 Picture/ 的 9 張）
  - 時鐘吃「絕對格號」→ 畫面 10 分鐘一輪重複 4 輪，時鐘仍連續走 08:00:00→08:40:00，
    **播放器不必開循環**（先前 10 分鐘循環的做法會讓時鐘每輪重來、08:20 之後就跟現場對不上）
  - 三層節奏拆開：大標 19.35s（600÷31 則）／照片 6.45s（大標 ÷3）／時鐘每秒。
    段落邊界用「累積四捨五入」算格號（`bounds()`），不會有累積誤差；
    每則大標的第一個畫面與換標必定同格 → 換標與換照片同步
  - 一個「畫面」= 一張橫幅照滿版 或 兩張直幅照並排；`build_playlist()` 每輪重新洗牌配對，
    同一對直幅照不會每次都黏在一起，並約每 5 個畫面插一張橫幅滿版
  - **主角取景**（`detect_subjects.swift` + `photo_crop()`）：
    大景照片（柱廊、大門、街景）主角又小又偏下，置中裁切會被標題帶吃掉 →
    用 macOS 內建 Vision 抓人臉框＋人體框（人臉在大景太小抓不到，人體框補得上；
    用系統框架就不必為產片裝 opencv／mediapipe 這類重依賴），
    裁窗依主角位置上下挪：放得下就整個人塞進「標題帶以上的可見區」、
    放不下就以頭部對齊（寧可裙襬被蓋到也不遮臉）。偵測結果快取 `Video/.focus_raw.json`
  - 效能：40 分鐘 = 7.2 萬格，單核要跑半小時 → 改 multiprocessing（fork，worker 靠 COW 共用底圖），
    分批合成讓記憶體上限固定、也不怕 worker 跑贏編碼器；實測 913% CPU、約 11 分鐘產完
  - Ken Burns 的逐格縮放改 BILINEAR（只縮 1.09 倍，與 LANCZOS 目視無差、快很多），
    一次性的預縮仍用 LANCZOS（4672px 原檔降到 1046px 是大幅降採樣，這裡才需要）
  - CRF 預設 19→23（40 分鐘約 720MB；實測 1:1 對比 crf 23 與 25 在文字色塊上看不出差別）
  - 大標 31 則：移除「改口成功」「奉茶禮成」「媒人今日最忙」，
    「交往紀錄正式歸檔」下一站改為「皇家薇庭 婚宴」；config 的 segments 簡化成 headlines 字串陣列
  - `.gitignore` 加 `Video/photos/`（照片原檔近百 MB，不可進 repo）
- 追加（第四輪回饋）：左下欄改成真實天氣輪播 + LIVE 閃爍紅點
  - 左下欄標籤「喜報」→「桃園區」，移除原本標籤下方那行「桃園區」與最底下的「9.12 婚宴」
  - **天氣區塊改成真實預報並與大標同步輪播**：
    新增 `Video/fetch_weather.py` 抓 9/12 全台 22 縣市（縣市政府座標）的最高/最低溫與 WMO 代碼，
    存成 `Video/weather.json`。用 Open-Meteo（免 API key，非氣象署官方）
    刻意落地成 json 而非產片時即時抓：①產片可重跑且結果一致 ②現場沒網路也能重產 ③臨近當天可再更新
    溫度**捨去小數**（32.9 → 32），不四捨五入
  - 天氣 icon 由原本單一太陽擴充成整組（sun / sun_cloud / cloud / fog / drizzle / rain / shower / thunder），
    仍全部程式畫（雲＝三顆圓＋一塊底，比貝茲曲線好調又夠像）；雨滴取 --wish-groom 系的藍提亮，不跳出婚禮色系
  - 天氣面板每縣市預先畫一張，逐格依大標索引 `hi % 縣市數` 疊上 → 換標與換縣市必定同一格
  - 右上角「文定 LIVE」→「LIVE」，置中於「分隔線→牌子右緣」之間，左邊空一格加紅色圓點；
    紅點做在逐格層（1 秒循環、亮 0.62 秒）才會閃，靜態框架層只留 LIVE 文字與位置
  - 照片停留時間 ×3：`screens_per_headline` 3→1 → 每張 6.45s→19.35s（與大標同步換）
- 追加（第五輪回饋）：保留兩層節奏但照片停久一點 → 一輪從 10 分改 20 分、輪數 4→2
  - 起因：上一輪為了「照片停留 ×3」把 `screens_per_headline` 設成 1，副作用是照片與大標同步、兩層節奏消失
  - 約束是算術性的：照片秒數 = (cycle_minutes×60 ÷ 大標則數) ÷ 每則張數。
    一輪 10 分 + 31 則 → 大標只有 19.35s，要兩層（張數≥2）照片就只剩 9.7s
  - 解法：一輪拉長成 20 分、輪數改 2（總長仍 40 分、時鐘照樣連續 08:00→08:40），
    每則 3 張 → **大標 38.71s／照片 12.90s**，兩層節奏保留且照片是原本 6.45s 的 2 倍
  - 驗證：t=12.7→13.4 只換照片（大標與縣市不動）；t=38.3→39.2 大標＋照片＋縣市三者同格換

---

## 2026-09-10 喜餅新增「數量」欄位（林蔡欽 ×3、陳思璇改西式）

婚前最後一次名單異動：對照紙本喜餅兌換名單，實質只有兩位要改——
- **林蔡欽（26091208）**：中式 1 盒 → **中式 3 盒**
- **陳思璇（26091232）**：中式 → **西式**（仍 1 盒）

（紙本上另有「新秘・新娘房」一份，不走線上兌換，不進系統；紙本與線上的姓名差異
一律以線上為準不動——姓名是 `sha256(salt+姓名)` 的輸入，改名等於換掉券號文件 id
與已寄出的 `?g=` 解鎖連結。）

### 為什麼要動到欄位而不是寫個備註
原本整條 pipeline 只有「喜餅類型」（`giftType`: chinese/western/both），**沒有數量概念**：
`verify.html` 的徽章寫死 `× 1`，盒數統計是用 `.length` 數人頭。林蔡欽的 3 盒無處可放，
只寫備註的話現場統計會少算 2 盒、也很可能只發 1 盒就放人走。故新增 `qty` 貫通全鏈。

### 改動
- **xlsx**（單一真實來源）新增 H 欄「喜餅數量」，有券號者填 1、林蔡欽 3；陳思璇類型改西式
- `import_firestore.py`：新增 `map_qty()`（未填／非法 → 1），讀「喜餅數量」欄，
  Firestore `vouchers` 文件加 `qty` 欄位（仍 merge 寫入，既有文件的 redeemed/redeemedAt 不動）
- `deploy_roster.py`：`voucher_map()` 把 qty 納入差異比對——**否則只改盒數不會觸發 Firestore 重匯**；
  報表加印「預備盒數：中式 N・西式 N」與多盒名單
- `verify.html`：`qtyOf()` 正規化；掃碼徽章 `× qty`、名單列徽章、名單統計改 `sumBox()` 盒數加總、
  CSV 匯出加「盒數」欄。**qty>1 比照西式跳醒目警示**（「這位要領 3 盒中式喜餅（不是 1 盒）」）——
  一次要發多盒是最容易漏發的情境
- `export_firestore.py`：報表加盒數欄、統計改盒數加總；順手修掉舊 bug——
  原本 `west = gift=="western"`、`cn = total - west`，「中式+西式」的人西式那盒完全沒被算到

### 盒數（線上 42 人）
中式 **42** 盒・西式 **5** 盒・合計 **47** 盒（＋紙本另計新秘 1 盒中式，線下處理）

### 補正（同日，獨立驗收後）
- `export_firestore.py` 統計頁的**未領取清單補上「盒數」欄**——現場點貨拿的就是這張，
  原本只有姓名/券號/樣式，林蔡欽看起來會跟其他人一樣是 1 盒
- `export_firestore.py` 收尾訊息單位混用（前兩個是人數、後兩個已改成盒數卻沒標單位）→ 改成
  「N 位女方賓客，已送出 N 位；盒數 中式 N/N 盒・西式 N/N 盒」；移除已無人使用的 `western` 殘留欄位
- `deploy_roster.py` **對「有填但解析失敗」的數量出聲警示**：`map_qty()` 會把「3盒」「三」「2.9」
  靜默吞成 1／截斷——而那正是本欄要防的錯，靜默失敗等於白做。validate() 現在會列出這些儲存格
